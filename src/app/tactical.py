...
for goal, (goal_x, goal_y) in goal_positions.items():
    field_grid[f"distance_to_{goal}_goal"] = np.sqrt(
        (field_grid["x_int"] - goal_x) ** 2 + (field_grid["y_int"] - goal_y) ** 2
    )
    field_grid[f"space_value_{goal}_goal"] = 1 - (
        field_grid[f"distance_to_{goal}_goal"]
        / np.sqrt(pitch_length**2 + pitch_width**2)
    )

games_lineups_df = pd.merge(lineups_df, games_df, on=["game.id"])
...
games_lineups_df = games_lineups_df[
    [
        "teamGame",
        "jerseyNum",
        "player.id",
...
tracking_df["x"] = tracking_df["x.raw"] + (pitch_length / 2)
tracking_df["y"] = tracking_df["y.raw"] + (pitch_width / 2)
tracking_df["z"] = tracking_df["z.raw"]

tracking_df["x_ball"] = np.where(
    tracking_df["teamGame"] == "balls", tracking_df["x"], np.nan
)
tracking_df["y_ball"] = np.where(
    tracking_df["teamGame"] == "balls", tracking_df["y"], np.nan
)
tracking_df["z_ball"] = np.where(
    tracking_df["teamGame"] == "balls", tracking_df["z"], np.nan
)
...
full_df = pd.merge(
    tracking_df, games_lineups_df, on=["jerseyNum", "teamGame"], how="left"
)
...
full_df["primaryColor"] = np.where(
    full_df["teamGame"] == "homePlayers",
    full_df["homeTeamKit.primaryColor"],
    np.where(
        full_df["teamGame"] == "awayPlayers",
...
full_df["text_ttp"] = np.where(
    full_df["teamGame"] == "balls",
    "Frame:"
    + full_df["frameNum"].astype("str")
    + "<br>Ball"
    + "<br>Visibility:"
    + full_df["visibility.raw"],
    "Frame:"
    + full_df["frameNum"].astype("str")
    + "<br>Player Name:"
    + full_df["player.nickname"]
    + "<br>Jersey Number:"
    + full_df["jerseyNum"].astype("str")
    + "<br>Position:"
    + full_df["positionGroupType"]
    + "<br>Visibility:"
    + full_df["visibility.raw"]
    + "<br>Confidence:"
    + full_df["confidence.raw"],
)
...
tactical.layout = dbc.Row(
    children=[
        dbc.Row(
            dcc.Markdown(
                f"## {homeTeamName} vs {awayTeamName}", style={"textAlign": "center"}
            )
        ),
        dbc.Row(
            justify="center",
                        ...
                        figure=go.Figure(
                            ...
                            layout=go.Layout(
                                ...
                                xaxis=dict(
                                    ...
                                    tickmode="array",
                                    tickvals=np.arange(
                                        xaxis_range[0] + 10, xaxis_range[0] - 9, 5
                                    ).tolist(),
...
def update_graph(sec, fig):
    ...
    # dash_player returns time in seconds, but the data is in ms, so we need to convert it
    milisec = sec * 1000
    ...
    field_frame = pd.merge(
        field_grid,
        frame_df[frame_df["teamGame"] == "balls"][
            ["x_int", "y_int", "x_ball", "y_ball"]
        ],
        on=["x_int", "y_int"],
        how="left",
    )
    ...
    field_frame["distance_to_ball"] = np.sqrt(
        (field_frame["x_int"] - field_frame["x_ball"]) ** 2
        + (field_frame["y_int"] - field_frame["y_ball"]) ** 2
    )
    field_frame["pass_probability"] = 1 - (
        field_frame["distance_to_ball"] / np.sqrt(pitch_length**2 + pitch_width**2)
    )

    for goal, _ in goal_positions.items():
        field_frame[f"pass_value_{goal}_goal"] = (
            field_frame["pass_probability"] * field_frame[f"space_value_{goal}_goal"]
        )

    field_frame["pass_value"] = (
        field_frame["pass_value_left_goal"] - field_frame["pass_value_right_goal"]
    )
